...
def chart_comparativa_indice_compuesto(df_res: pd.DataFrame):
    """Gráfico ejecutivo de líneas suaves con proyecciones y meta 85% para el Índice Compuesto Institucional."""
    rule_df = pd.DataFrame({'meta': [85.0], 'label': ['Meta Institucional: 85%']})
    rule = alt.Chart(rule_df).mark_rule(color='#ef4444', strokeWidth=1.5, strokeDash=[4, 4]).encode(y='meta:Q')
    ...
    base = alt.Chart(df_res).encode(
        x=alt.X('bimestre:N', title=None, sort=['B1', 'B2', 'B3', 'B4', 'B5'], axis=alt.Axis(
            labelAngle=0, 
            labelFontSize=12, 
            labelFontWeight='bold',
            grid=False,
        ...
        y=alt.Y('indice:Q', title='Índice Compuesto (%)', scale=alt.Scale(domain=[65, 95]), axis=alt.Axis(
            format='.0f',
            grid=True,
            gridColor='#f1f5f9',
            gridWidth=1,
        ...
        opacity=alt.Opacity('tipo:N', scale=alt.Scale(
            domain=['Real', 'Proyección'],
            range=[1.0, 0.55]
        ), legend=None),
        tooltip=[
    ...
    # Un solo marcador por línea en el punto de transición B3 (donde termina dato real y empieza proyección)
    point_b3 = base.transform_filter("datum.bimestre == 'B3' && datum.tipo == 'Real'").mark_circle(size=70)

    return (lines + point_b3 + rule + rule_text).properties(height=420).configure_view(stroke='transparent')
